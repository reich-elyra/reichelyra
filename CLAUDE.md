...
- **Login + back button bar (2026-07-11, redesigned same day after user feedback)** — `UTILITY_BAR_HTML`, a full-width `position:fixed` bar flush to the bottom edge (back "→ رجوع" outline button + gold "تسجيل الدخول" link to `/auth/login`), injected only on pages with the marketing nav (`.mnav-inner` detected → `hasMarketingNav` flag). A matching `<style>body{padding-bottom:${BAR_HEIGHT}px !important}</style>` is injected alongside it so the bar never overlaps real page content (first version was a floating center pill with no reserved space — user reported it looked unpolished and overlapped the hero's own CTA buttons). **Critical constraint discovered here: Z App is a SvelteKit app that hydrates client-side and wipes any extra DOM nodes injected *inside* its component tree** (verified: elements added inside `.mnav-inner` render in the raw HTTP response but vanish within ~1s once JS hydrates — confirmed via `chrome --headless --dump-dom`, which captures the DOM *after* JS runs, unlike `curl`/`--screenshot`). The fix: inject via `onDocument`'s `end` handler so the bar (and its `<style>` tag — CSS applies regardless of where in the document it's defined) lands as a sibling of SvelteKit's mount div (`<div style="display: contents">`) instead of inside it — siblings of that div survive hydration untouched. **Any future DOM injection into this origin must be verified with `--dump-dom`, not just `curl` — curl only shows pre-hydration SSR output and will falsely appear to work.**
