...
- **Floating login + back button (2026-07-11)** — `UTILITY_CLUSTER_HTML`, a `position:fixed` bottom-center pill (back "→" button + gold "تسجيل الدخول" link to `/auth/login`), injected only on pages that have the marketing nav (`.mnav-inner` detected → `hasMarketingNav` flag). **Critical constraint discovered here: Z App is a SvelteKit app that hydrates client-side and wipes any extra DOM nodes injected *inside* its component tree** (verified: elements added inside `.mnav-inner` render in the raw HTTP response but vanish within ~1s once JS hydrates — confirmed via `chrome --headless --dump-dom`, which captures the DOM *after* JS runs, unlike `curl`/`--screenshot`). The fix: inject via `onDocument`'s `end` handler so the cluster lands as a sibling of SvelteKit's mount div (`<div style="display: contents">`) instead of inside it — siblings of that div survive hydration untouched. **Any future DOM injection into this origin must be verified with `--dump-dom`, not just `curl` — curl only shows pre-hydration SSR output and will falsely appear to work.**
