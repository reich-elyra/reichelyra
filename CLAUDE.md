...
- **Login badge (top) + back bar (bottom) — split layout, 2026-07-11, iterated twice on user feedback:**
  - v1: floating center pill at the bottom with both buttons — user said it looked unpolished and overlapped the hero's own CTA buttons.
  ...
  - v3 (current): `LOGIN_BADGE_HTML` is a small gold pill fixed at `top:70px;right:14px` (just below the nav row) linking to `/auth/login`; `BACK_BAR_HTML` is the full-width bottom bar (unchanged) with just the "→ رجوع" button. **First attempt at v3 placed the login badge at `top:14px` — overlapping/covering part of the "REICH ELYRA" wordmark, since the brand text+logo extends further right than initially estimated from screenshots.** Fixed by moving it below the nav row (top:70px) instead of trying to slot into a horizontal gap beside the logo — always verify nav-area placement with an actual screenshot, don't estimate pixel gaps from a single crop.
  - Both survive hydration via the same `onDocument` `end`-handler sibling-injection technique (see below) — **critical constraint discovered here: Z App is a SvelteKit app that hydrates client-side and wipes any extra DOM nodes injected *inside* its component tree** (verified: elements added inside `.mnav-inner` render in the raw HTTP response but vanish within ~1s once JS hydrates — confirmed via `chrome --headless --dump-dom`, which captures the DOM *after* JS runs, unlike `curl`/`--screenshot`). The fix: inject via `onDocument`'s `end` handler so the elements (and the `<style>body{padding-bottom:...}</style>` tag reserving space for the bottom bar — CSS applies regardless of where in the document it's defined) land as a sibling of SvelteKit's mount div (`<div style="display: contents">`) instead of inside it — siblings of that div survive hydration untouched. **Any future DOM injection into this origin must be verified with `--dump-dom` AND a real screenshot, not just `curl` — curl only shows pre-hydration SSR output and will falsely appear to work; a single-crop screenshot can also mislead on exact element widths.**
